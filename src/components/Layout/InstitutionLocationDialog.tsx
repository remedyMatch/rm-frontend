...
const InstitutionLocationDialog: React.FC<Props> = (props) => {
  ...
  >(person?.aktuellerStandort.institution);
  const [selectedLocation, setSelectedLocation] = useState<
    InstitutionStandort | undefined
  >(person?.aktuellerStandort.standort);
  ...
  const onSelect = useCallback(async () => {
    ...
    if (result.error) {
      logApiError(
        result,
        "Beim Auswählen der Institution und des Standorts ist ein Fehler aufgetreten"
      );
      setError(
          ...
          {person?.institutionen.map((entry) => (
            <div
              ...
              onClick={
                disabled
                  ? undefined
                  : () => setSelectedInstitution(entry.institution)
              }
              className={clsx(
                classes.entry,
                entry.institution.id === selectedInstitution?.id &&
                  classes.entrySelected
              )}
            >
              ...
              <div className={classes.entryTextBlock}>
                <span className={classes.institutionName}>
          ...
          {person?.institutionen
            ...
            ?.standorte.map((entry) => (
              <div
                ...
                className={clsx(
                  classes.entry,
                  entry.id === selectedLocation?.id && classes.entrySelected
                )}
              >
                <LocationOn fontSize="large" />
                <div className={classes.entryTextBlock}>
                  <span className={classes.locationName}>{entry.name}</span>
                  <span className={classes.locationStreet}>
                    {entry.strasse} {entry.hausnummer}
                  </span>
                  <span className={classes.locationCity}>
                    {entry.plz} {entry.ort}
                  </span>
                </div>
              </div>
