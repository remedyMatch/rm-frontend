...
const MatchList: React.FC<Props> = (props) => {
  ...
  const items = props.results.slice(
    pageIndex * PAGE_SIZE,
          ...
          <div className={classes.resultItem}>
            <div className={classes.article}>
              <span className={classes.adType}>
                {capitalize(item.original.inseratTyp.toLowerCase())}
              </span>
              <img
                src={item.icon}
                alt="Kategorie"
                className={classes.articleIcon}
              />
              <span className={classes.articleName}>
                {item.original.artikel.name}
              </span>
              {item.original.artikel.varianten.length > 1 && (
                <span className={classes.variantName}>
                  Gr.{" "}
                  {
                    item.original.artikel.varianten?.find(
                      (v) => v.id === item.original.artikelVarianteId
                    )?.variante
                  }
                </span>
              )}
            ...
            <div className={classes.locationColumn}>
              ...
              <span className={classes.locationDetails}>
                {item.original.standortVon.strasse}{" "}
                {item.original.standortVon.hausnummer}
              </span>
              <span className={classes.locationDetails}>
              ...
              <span className={classes.locationDetails}>
                {
                  countries.DE[
                    item.original.standortVon.land as keyof typeof countries.DE
                  ]
                }
              </span>
            </div>
            <div className={classes.locationColumn}>
              ...
              <span className={classes.locationDetails}>
                {item.original.standortAn.strasse}{" "}
                {item.original.standortAn.hausnummer}
              </span>
              <span className={classes.locationDetails}>
              ...
              <span className={classes.locationDetails}>
                {
                  countries.DE[
                    item.original.standortAn.land as keyof typeof countries.DE
                  ]
                }
              </span>
            </div>
