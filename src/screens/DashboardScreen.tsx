...
class DashboardScreen extends Component<Props, State> {
  ...
  render() {
    ...
    const ownCount =
      (this.props.institutionAngebote?.length || 0) +
      (this.props.institutionBedarfe?.length || 0);
        ...
        <div className={classes.welcomeArea}>
          <div className={classes.welcomeAreaLeft}>
            <Typography className={classes.welcome}>
              Hallo, {this.props.person?.vorname}&nbsp;
              {this.props.person?.nachname}
            </Typography>
            ...
            {this.mapAds()
              ...
              .map((entry) => (
                <div
                  ...
                  onClick={() =>
                    this.props.history.push("/inserate/" + entry.id)
                  }
                >
                  {entry.requests > 0 && (
            ...
            {mapConversations(
              ...
              .map((entry) => (
                <div
                  className={classes.adEntry}
                  onClick={() =>
                    this.props.history.push("/konversation/" + entry.id)
                  }
                >
  ...
  componentDidUpdate(prevProps: Readonly<Props>) {
    if (prevProps.konversationen !== this.props.konversationen) {
      this.props.loadKonversationAngebotAnfragen(
        ...getOfferRequestIds(this.props.konversationen || [])
      );
      this.props.loadKonversationBedarfAnfragen(
        ...getDemandRequestIds(this.props.konversationen || [])
      );
    }
  }
  ...
  private mapAds = () => {
    ...
    return bedarfe
      .map((bedarf) => this.mapToAd(bedarf, true))
...
const mapDispatchToProps = (dispatch: RootDispatch) => ({
  ...
  loadKonversationAngebotAnfragen: (...ids: string[]) =>
    dispatch(loadKonversationAngebotAnfragen(...ids)),
  loadKonversationBedarfAnfragen: (...ids: string[]) =>
    dispatch(loadKonversationBedarfAnfragen(...ids)),
