...
const OfferFlowScreen: React.FC = () => {
  ...
  const getResultsTitle = useCallback(
    (
      ...
      const article = getArticleName(selectedArticle, selectedVariant);
      if (resultCount === 0)
        return `Wir haben keine Bedarfe für ${article} gefunden.`;
      if (resultCount === 1)
        return `Wir haben 1 Bedarf für ${article} gefunden.`;
  ...
  const getResultsSubtitle = useCallback((resultCount: number) => {
    if (resultCount === 0)
      return "Sie können aber ein Inserat erstellen, damit sich Empfänger bei Ihnen melden können.";
  ...
  return (
    <PPEFlow
      ...
      getLoadArticleCountsUrl={(categoryId) =>
        "/remedy/bedarf/suche/filter/artikel?ohneEigene=true&artikelKategorieId=" +
        categoryId
      }
      getLoadCategoryCountsUrl={() =>
        "/remedy/bedarf/suche/filter/artikelkategorie?ohneEigene=true"
      }
      getLoadingPageTitle={getLoadingPageTitle}
      getLoadResultsUrl={(variantId) =>
        "/remedy/bedarf/suche?ohneEigene=true&artikelVarianteId=" + variantId
      }
      getLoadVariantCountsUrl={(articleId) =>
        "/remedy/bedarf/suche/filter/artikelvariante?ohneEigene=true&artikelId=" +
        articleId
      }
