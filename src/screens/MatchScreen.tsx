  ...
  const entries = useMemo(
    () =>
      matches?.map((match) => ({
        original: match,
        icon: getIcon(
          kategorien?.find((k) => k.id === match.artikel.artikelKategorieId)
            ?.name || ""
        ),
        conversationId: conversations?.find(
          (c) => c.referenzId === match.anfrageId
        )?.id,
      })) || [],
    ...
    <>
      <div className={classes.container}>
        ...
        <Typography className={classes.subtitle}>
          Unten sehen Sie alle Matches, also alle Anfragen, die Sie gestellt
          oder empfangen haben, die akzeptiert wurden.
        </Typography>

        <MatchList
          results={entries}
          onOpenConversationClicked={(item) =>
            history.push("/konversation/" + item.conversationId || "")
          }
        />
      </div>
    </>
