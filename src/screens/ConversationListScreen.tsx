...
const ConversationListScreen: React.FC = () => {
  ...
  useEffect(() => {
    const angebotAnfrageIdsToLoad = conversations
      .filter((k) => k.referenzTyp === "ANGEBOT_ANFRAGE")
      .map((k) => k.referenzId);
    const bedarfAnfrageIdsToLoad = conversations
      .filter((k) => k.referenzTyp === "BEDARF_ANFRAGE")
      .map((k) => k.referenzId);
  ...
  const onOpenConversationClicked = useCallback(
  ...
  return (
    <>
      <div className={classes.container}>
        <Typography className={classes.title}>Ihre Konversationen</Typography>
        <Typography className={classes.subtitle}>
          Unten sehen Sie alle Ihre vorhandenen Konversationen aufgelistet. Eine
          Konversation wird erstellt, sobald Sie eine Anfrage zu einem Angebot
          oder Bedarf stellen oder erhalten.
        </Typography>
