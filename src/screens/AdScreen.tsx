...
const mapToAd = (
  ...
  return {
    id: entry.id,
    icon: getIcon(
      kategorien?.find((ak) => ak.id === entry.artikel.artikelKategorieId)
        ?.name || ""
    ),
    articleName: entry.artikel.name,
    variantName:
      entry.artikel.varianten.length > 1
        ? entry.artikel.varianten.find((v) => v.id === entry.artikelVarianteId)
            ?.variante
        : undefined,
    ...
    sealed:
      ("originalverpackt" in entry && entry.originalverpackt) || undefined,
    sterile: entry.steril,
    medical: entry.medizinisch,
    useBefore:
      ("haltbarkeit" in entry &&
        !!entry.haltbarkeit &&
        new Date(entry.haltbarkeit)) ||
      undefined,
    original: entry,
    ...
    requests: entry.anfragen.map((request) => ({
      ...
      conversationId: konversationen?.find((k) => k.referenzId === request.id)
        ?.id,
    })),
  };
};
...
const AdScreen: React.FC = () => {
  ...
  const [selectedAdType, setSelectedAdType] = useState<
    "offer" | "demand" | undefined
  >(undefined);
  const [selectedAd, setSelectedAd] = useState<Angebot | Bedarf | undefined>(
    undefined
  );

  const onFindAdClicked = useCallback(
    (item: AdListDataRow) => {
      if (item.type === "demand") {
        history.push(
          `/bedarf/${item.categoryId}/${item.articleId}/${item.variantId}`
        );
      } else if (item.type === "offer") {
        history.push(
          `/angebot/${item.categoryId}/${item.articleId}/${item.variantId}`
        );
      }
    },
    [history]
  ...
  const onEditCountDialogSaved = useCallback(() => {
    dispatch(
      selectedAdType === "offer"
        ? loadInstitutionAngebote()
        : loadInstitutionBedarfe()
    );
  ...
  const angebote = useSelector(
    (state: RootState) => state.institutionAngebote.value
  );
  const bedarfe = useSelector(
    (state: RootState) => state.institutionBedarfe.value
  );
  const kategorien = useSelector(
    (state: RootState) => state.artikelKategorien.value
  );
  ...
  const ads = useMemo(
    () =>
      (angebote || [])
        ...
        .concat(
          bedarfe?.map((x) => mapToAd(x, true, kategorien, konversationen)) ||
            []
        )
    ...
    <>
      <div className={classes.container}>
        <Typography className={classes.title}>
          {match.params.adId ? "Inseratsdetails" : "Ihre Inserate"}
        </Typography>
