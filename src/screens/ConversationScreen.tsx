...
const ConversationScreen: React.FC = () => {
  ...
  const conversations =
    useSelector((state: RootState) => state.konversationen.value) || [];
  const person = useSelector((state: RootState) => state.person.value);
  const conversationDemandDetails = useSelector(
    (state: RootState) => state.konversationBedarfAnfragen.value
  );
  const conversationOfferDetails = useSelector(
    (state: RootState) => state.konversationAngebotAnfragen.value
  );

  const [conversation, setConversation] = useState<Konversation | undefined>(
    conversations.find((k) => k.id === match.params.conversationId)
  );
  ...
  const refreshData = useCallback(async () => {
    const result = await apiGet<Konversation>(
      "/remedy/konversation/" + match.params.conversationId
    );
    if (!result.error) {
  ...
  const onSendClicked = useCallback(async () => {
    ...
    const result = await apiPost(
      "/remedy/konversation/" + match.params.conversationId + "/nachricht",
      {
  ...
  const articleName = request
    ? ad?.artikel.name +
      ((variants?.length || 0) > 1 ? " (" + variant?.variante + ")" : "")
    : "";
  const mine =
    request?.institution.id === person?.aktuellerStandort.institution.id;
  const title =
    (mine
      ? "Ihre Anfrage "
      : "Anfrage von " + (request?.institution.name || "???")) +
    " zu " +
    (conversation?.referenzTyp === "ANGEBOT_ANFRAGE" ? "Angebot" : "Bedarf") +
    ": " +
    (ad?.verfuegbareAnzahl || "???") +
    " " +
    (articleName || "");

  const requestAction = useCallback(
    async (action: "accept" | "dismiss" | "cancel") => {
      setInputDisabled(true);
      const type =
        conversation?.referenzTyp === "ANGEBOT_ANFRAGE" ? "angebot" : "bedarf";
      const result = await apiPost(
        `/remedy/${type}/${ad?.id}/anfrage/${conversation?.referenzId}/${
          action === "cancel" ? "stornieren" : "beantworten"
        }`,
        action !== "cancel"
          ? {
  ...
  const onCancelRequestClicked = useCallback(() => requestAction("cancel"), [
    requestAction,
  ]);
  const onAcceptRequestClicked = useCallback(() => requestAction("accept"), [
    requestAction,
  ]);
  const onDismissRequestClicked = useCallback(() => requestAction("dismiss"), [
    requestAction,
  ]);
