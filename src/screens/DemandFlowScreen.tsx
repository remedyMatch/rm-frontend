...
const DemandFlowScreen: React.FC = () => {
  ...
  const getArticleName = useCallback(
    (article?: Artikel, variant?: ArtikelVariante) => {
  ...
  const getLoadingPageTitle = useCallback(
    (selectedArticle?: Artikel, selectedVariant?: ArtikelVariante) => {
  ...
  const getResultsTitle = useCallback(
    (
      ...
      const article = getArticleName(selectedArticle, selectedVariant);
      if (resultCount === 0)
        return `Wir haben keine Angebote für ${article} gefunden.`;
      if (resultCount === 1)
        return `Wir haben 1 Angebot für ${article} gefunden.`;
  ...
  const getResultsSubtitle = useCallback((resultCount: number) => {
    if (resultCount === 0)
      return "Sie können aber ein Inserat erstellen, damit sich Spender bei Ihnen melden können.";
  ...
  return (
    <PPEFlow
      ...
      getLoadArticleCountsUrl={(categoryId) =>
        "/remedy/angebot/suche/filter/artikel?ohneEigene=true&artikelKategorieId=" +
        categoryId
      }
      getLoadCategoryCountsUrl={() =>
        "/remedy/angebot/suche/filter/artikelkategorie?ohneEigene=true"
      }
      getLoadingPageTitle={getLoadingPageTitle}
      getLoadResultsUrl={(variantId) =>
        "/remedy/angebot/suche?ohneEigene=true&artikelVarianteId=" + variantId
      }
      getLoadVariantCountsUrl={(articleId) =>
        "/remedy/angebot/suche/filter/artikelvariante?ohneEigene=true&artikelId=" +
        articleId
      }
