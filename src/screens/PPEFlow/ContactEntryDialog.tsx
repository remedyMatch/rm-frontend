  ...
  const onContact = useCallback(async () => {
    const error = validate(
      defined(
        selectedAd,
        "Es wurde kein " +
          (type === "offer" ? "passendes Angebot" : "passender Bedarf") +
          " ausgewählt!"
      ),
      ...
      numberSize(
        amount!,
        "Die " + (type === "offer" ? "angebotene" : "angefragte") + " Anzahl",
        1,
        Math.min(
          selectedAd!.verfuegbareAnzahl,
          selectedEntry!.verfuegbareAnzahl
        )
      ),
    ...
    if (type === "offer") {
      result = await apiPost(
        "/remedy/bedarf/" + selectedEntry!.id + "/anfrage",
        {
          anzahl: amount,
          nachricht: comment,
      ...
      result = await apiPost(
        "/remedy/angebot/" + selectedEntry!.id + "/anfrage",
        {
          anzahl: amount,
          nachricht: comment,
  ...
  return (
    <>
      <PopupDialog
        ...
        subtitle={
          "Bitte geben Sie die Anzahl, die Sie " +
          (type === "offer" ? "anbieten" : "anfragen") +
          " wollen, sowie Ihre Nachricht unten ein und klicken Sie auf 'Anfrage absenden'."
        }
      >
        <div className={classes.dialogContent}>
          <NumberInput
            ...
            label={
              type === "offer"
                ? "Wie viele wollen Sie anbieten?"
                : "Wie viele wollen Sie anfragen?"
            }
            disabled={disabled}
            onChange={setAmount}
            value={amount}
