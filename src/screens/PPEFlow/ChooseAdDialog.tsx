...
const ChooseAdDialog: React.FC<Props> = (props) => {
  ...
  const [selectedEntry, setSelectedEntry] = useState<
    InstitutionAngebot | InstitutionBedarf | undefined
  >(undefined);

  const matchingEntries = props.institutionEntries.filter(
    (entry) => entry.artikelVarianteId === props.variantId
  );
  if (matchingEntries.length === 0) {
    return (
      <PopupDialog
        ...
        secondTitle={
          props.flowType === "offer" ? "Angebot erstellen" : "Bedarf erstellen"
        }
  ...
  return (
    <>
      <PopupDialog
        ...
        onFirst={props.onCancelled}
        secondTitle={
          props.flowType === "offer" ? "Angebot auswählen" : "Bedarf auswählen"
        }
        ...
        <div className={classes.entries}>
          {matchingEntries.map((entry) => (
            <div
              ...
              className={clsx(
                classes.entry,
                entry === selectedEntry && classes.entrySelected
              )}
            >
              <img
              ...
              />
              <span className={classes.amountCount}>
                {entry.verfuegbareAnzahl}x
              </span>
              <span className={classes.articleName}>{entry.artikel.name}</span>
              {entry.artikel.varianten.length > 1 && (
                <span className={classes.variantName}>
                  Gr.{" "}
                  {
                    entry.artikel.varianten.find(
                      (variante) => variante.id === props.variantId
                    )?.variante
                  }
                </span>
              )}
