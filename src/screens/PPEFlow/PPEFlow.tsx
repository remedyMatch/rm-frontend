...
import ResultList, {
  ResultListDataRow,
} from "../../components/List/ResultList";
...
class PPEFlow extends Component<Props, State> {
    ...
    return (
      <>
        ...
        <ChooseAdDialog
          ...
          institutionEntries={
            (this.props.flowType === "offer"
              ? this.props.institutionAngebote
              : this.props.institutionBedarfe) || []
          }
  ...
  componentDidMount = () => {
    ...
    if (
      this.props.initialCategoryId &&
      this.props.initialArticleId &&
  ...
  componentDidUpdate() {
    if (
      this.props.initialCategoryId &&
      this.props.artikelKategorien &&
      !this.state.initialCategoryProcessed
    ) {
      const category = this.props.artikelKategorien.find(
        (k) => k.id === this.props.initialCategoryId
      );
      if (category) {
        this.setState({
    ...
    if (
      this.props.initialArticleId &&
      this.props.artikel &&
      !this.state.initialArticleAndVariantProcessed
    ) {
      const article = this.props.artikel.find(
        (a) => a.id === this.props.initialArticleId
      );
      if (article) {
        if (this.props.initialVariantId) {
          const variant = article.varianten.find(
            (v) => v.id === this.props.initialVariantId
          );
          if (variant) {
            this.setState({
  ...
  private getArticleCategoryPage = () => {
    return {
      ...
      contentClass: clsx(
        this.props.classes.elementContainer,
        this.props.classes.categoryContainer
      ),
      content: this.props.artikelKategorien?.map((category) => (
        ...
        >
          <CountBadge
            ...
            count={
              this.state.categoryCounts.find(
                (count) => count.id === category.id
              )?.anzahl
            }
          />
          <img
  ...
  private getArticlePage = () => {
    ...
    const articles = this.props.artikel?.filter(
      (artikel) =>
        artikel.artikelKategorieId === this.state.selectedCategory?.id
    );
    const className = clsx(
      this.props.classes.elementContainer,
      this.props.classes.articleContainer
    );
    ...
    return {
      ...
      content: articles?.map((article) => {
        const count = this.state.articleCounts.find(
          (count) => count.id === article.id
        )?.anzahl;
  ...
  private getArticleVariantPage = () => {
    ...
    return {
      ...
      content: variants?.map((variant) => {
        const count = this.state.variantCounts.find(
          (count) => count.id === variant.id
        )?.anzahl;
  ...
  private getFinishedPage = () => {
    ...
    if (this.state.resultsLoading) {
      return {
        title: this.props.getLoadingPageTitle(
          this.state.selectedArticle,
          this.state.variantSkipped ? undefined : this.state.selectedVariant
        ),
    ...
    return {
      title: this.props.getResultsPageTitle(
  ...
  private onChooseAdDialogChosen = (
    ad: InstitutionAngebot | InstitutionBedarf
  ) => {
    this.setState({
      chooseAdDialogOpen: false,
      selectedAd: ad,
  ...
  private onCreateAdDialogCreated = (
    ad: InstitutionAngebot | InstitutionBedarf
  ) => {
    this.setState({
      selectedAd: ad,
  ...
  private mapResults = () => {
    return this.state.results.map((result) => ({
      ...
      sealed:
        ("originalverpackt" in result && result.originalverpackt) || undefined,
      sterile: result.steril,
      medical: result.medizinisch,
      useBefore:
        ("haltbarkeit" in result &&
          !!result.haltbarkeit &&
          new Date(result.haltbarkeit)) ||
        undefined,
      original: result,
      type:
        this.props.flowType === "offer"
      ...
      buttonText:
        this.props.flowType === "offer"
          ...
          : "Spender kontaktieren",
    }));
  };

  private loadCategoryCounts = async () => {
    const result = await apiGet<CountEntry[]>(
      this.props.getLoadCategoryCountsUrl()
    );
    if (result.error) {
      logApiError(
        result,
        "Beim Laden der verfügbaren Inserate für die Artikel-Kategorien ist ein Fehler aufgetreten"
      );
    } else {
      this.setState({
  ...
  private loadArticleCounts = async (categoryId?: string) => {
    ...
    const result = await apiGet<CountEntry[]>(
      this.props.getLoadArticleCountsUrl(categoryId)
    );
    if (result.error) {
      logApiError(
        result,
        "Beim Laden der verfügbaren Inserate für die Artikel ist ein Fehler aufgetreten"
      );
    } else {
      this.setState({
  ...
  private loadVariantCounts = async (articleId?: string) => {
    ...
    const result = await apiGet<CountEntry[]>(
      this.props.getLoadVariantCountsUrl(articleId)
    );
    if (result.error) {
      logApiError(
        result,
        "Beim Laden der verfügbaren Inserate für die Artikel-Varianten ist ein Fehler aufgetreten"
      );
    } else {
      this.setState({
  ...
  private loadResults = async (variantId: string) => {
    ...
    const result = await apiGet<(Angebot | Bedarf)[]>(
      this.props.getLoadResultsUrl(variantId)
    );
  ...
  private updateRoute = () => {
    ...
    if (this.state.currentPage === 0) {
      ...
      this.props.history.push(
        `${prefix}/${this.state.selectedCategory?.id}/${this.state.selectedArticle?.id}`
      );
    } else if (this.state.currentPage === 3) {
      this.props.history.push(
        `${prefix}/${this.state.selectedCategory?.id}/${this.state.selectedArticle?.id}/${this.state.selectedVariant?.id}`
      );
    }
  };
}
